...
class Tweener {
    ...
    private static void remove(Animator animator) {
        ...
        while (iter.hasNext()) {
            ...
            if (entry.getValue().animator == animator) {
                if (DEBUG)
                    Log.v(TAG,
                            "Removing tweener " + sTweens.get(entry.getKey())
    ...
    public static Tweener to(Object object, long duration, Object... vars) {
        ...
        for (int i = 0; i < vars.length; i += 2) {
            ...
            if ("simultaneousTween".equals(key)) {
                ...
                props.add(PropertyValuesHolder.ofInt(key, ((int[]) value)[0],
                        ((int[]) value)[1]));
            } else if (value instanceof Number) {
                float floatValue = ((Number) value).floatValue();
                props.add(PropertyValuesHolder.ofFloat(key, floatValue));
            } else {
                throw new IllegalArgumentException("Bad argument for key \""
                        + key + "\" with value " + value.getClass());
            }
        }
    ...
    private static void replace(ArrayList<PropertyValuesHolder> props,
                                Object... args) {

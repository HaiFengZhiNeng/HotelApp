...
import com.fanfan.hotel.serial.listener.OnOpenSerialPortListener;
import com.fanfan.hotel.serial.listener.OnSerialPortDataListener;
import com.fanfan.hotel.serial.thread.SerialPortReadThread;
...
public class SerialPortManager extends SerialPort {
    ...
    public boolean openSerialPort(File device, int baudRate) {
        ...
        Print.e("openSerialPort: " + String.format("打开串口 %s  波特率 %s", device.getPath(), baudRate));

        // 校验串口权限
        if (!device.canRead() || !device.canWrite()) {
            boolean chmod777 = chmod777(device);
            if (!chmod777) {
                Print.e("openSerialPort : " + baudRate + " 没有读写权限");
